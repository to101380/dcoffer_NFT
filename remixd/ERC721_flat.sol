...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    mapping(uint => uint) private _dcofferPower;
    ...
    function dcofferPower(uint tokenId)public view returns(uint){
        return _dcofferPower[tokenId];
    }

    function powerBalances(address owner)public view returns(uint){
        return _powerBalances[owner];
    }
    ...
    function _set_power(address owner,uint tokenId)private {               
        uint random = _baseNumber.add(_baseNumber.mul(tokenId)).add(tokenId);
        ...
        _dcofferPower[tokenId] = power;
        _powerBalances[owner] = _powerBalances[owner].add(power);        
             
    } 



    function setBaseNumber(uint number)external onlyOwner{
        _baseNumber = number;
    } 


    function setblock(address owner)private{

        if(_lastblock[owner] == 0){
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        uint power = _dcofferPower[tokenId];
        _powerBalances[owner] = _powerBalances[owner].sub(power);
        ...
        _totalPower = _totalPower.sub(power); 
    }
    ...
    function _transfer(
        ...
        _powerBalances[from] = _powerBalances[from].sub(_dcofferPower[tokenId]);
        _powerBalances[to] = _powerBalances[to].add(_dcofferPower[tokenId]);
...
abstract contract _ERC721Enumerable is ERC721Enumerable{
    function totalSupply() public pure returns (uint256){
        uint256 _totalSupply = 13122;
        return _totalSupply;
    }
}
...
contract Dcoffer is ERC721, ERC721URIStorage, ERC721Burnable, _ERC721Enumerable {
    using SafeMath for uint;
    ...
    {   
        if(tokenId >= unlock_blind){
            return "https://gateway.pinata.cloud/ipfs/QmXajCH9aypkBKPwQDAB7SRGsVDKmGWkfW3SrPxQAFgZFg";
        }else{
            return super.tokenURI(tokenId);
        }
    ...
    function setUnlockBlind(uint _tokenID)external onlyOwner{
        unlock_blind = _tokenID;
    }
